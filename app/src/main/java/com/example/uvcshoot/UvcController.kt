...
class UvcController(
    ...
    private val captureFrameListener = object {
        ...
        fun onMjpegFrame(jpegBytes: ByteArray, width: Int, height: Int) {
            Log.d("UVC", "captureFrameListener: bytes=${jpegBytes.size} width=$width height=$height")
            // Disable further delivery after receiving the capture frame
            if (nativeHandle != 0L) {
                NativeBridge.nativeSetDeliverFramesToJava(nativeHandle, false)
            }
            // TODO: handle captured JPEG bytes (e.g. save to file or upload)
        }
    }
    ...
    init {
        previewSurface.holder.addCallback(this)
    }
    ...
    override fun surfaceChanged(holder: SurfaceHolder, format: Int, width: Int, height: Int) {
        Log.d("UVC", "surfaceChanged: ${width}x${height}")
    }
    ...
    fun onResume() {
        ...
        try {
            ...
            // Pass surface if already available
            val surface: Surface? = previewSurface.holder.surface.takeIf { it.isValid }
            if (surface != null) {
